...
#define GetUjKStruc( dBtag, Jtag, Btag, invfn, alpmat, niint, timmat, eps ) \
 ...
 #else
 		REprintf( "MC++: -= encounters improp dimensioned args. Die.\n");
 		return;
 #endif
 		}
 ...
 *curtmp = detsc;
 ...
 matrix vec2diag( matrix& x )
 	...
 	if ( x.cols() != 1 && x.rows() != 1 )
 		{
 		REprintf("MC++: vec2diag: need arg with 1 row or 1 col\n");
 #ifndef FOR_S
...
*/
 ...
 // for certain corcodes you can get dUda, sum uu' for s.e. alp
    ...
    S3 = 0.*S3;
    S5 = 0.*S5;
 if (*verbose) REprintf("starting iter %d\n",iter);
    for (int i = 0 ; i < I; i++)
      {
...
//return(0);
	...
	//	return(0);
		...
		REprintf("some problem! foo=%d",foo);
		        return out12;
		        }
